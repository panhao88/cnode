      ...
      axios
        ...
        .then(res => {
          res.data.data.map(item => {
            ...
            let now = Date.now()
            item.second = Math.floor((now - item.time) / 1000)
            item.mins = Math.floor((now - item.time) / 1000 / 60)
            item. hour= Math.floor((now - item.time) / 1000 / 60 / 60) 
            item. day = Math.floor((now - item.time) / 1000 / 60 / 60 / 24) 
            this.$set(item, 'second', Math.ceil((now - item.time) / 1000))
            this.$set(item, 'mins', Math.ceil((now - item.time) / 1000 / ))
            this.$set(item, 'hour', Math.ceil((now - item.time) / 1000))
            this.$set(item, 'day', Math.ceil((now - item.time) / 1000))
          })
