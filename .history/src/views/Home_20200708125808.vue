  ...
  <div>
   ...
   <div>
      ...
      <tr v-for="(item,index) in arr.slice((currentPage - 1) * pageSize, currentPage*pageSize) " :key="index"  class="bordera">
       ...
       <div class="img3">
          <td v-if="item.day && item.day < 30&&item.day >=1">{{Math.ceil(item.day)}}天前</td>
          <td v-if="item.day && item.day >= 30">{{parseInt(item.day / 30)}}月前</td>
           <td v-if="item.day && item.day/24 > 0 && item.day/24 < 24">{{Math.ceil(item.day/24)}}小时前</td>
       </div>
      </tr>
    </table>
     <el-pagination
