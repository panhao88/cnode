  ...
  <div>
   ...
   <div>
      ...
      <tr v-for="(item,index) in arr.slice((currentPage - 1) * pageSize, currentPage*pageSize) " :key="index"  class="bordera">
       ...
       <div class="img3">
          <td v-if="item.day && item.day < 30&&item.hour <=24">{{item.day}}天前</td>
          <td v-if="item.day && item.day >= 30">{{item.day % 30}}月前</td>
           <td v-if="item.hour && item.hour > 1 && item.hour < 24">{{item.hour % 24}}小时前</td>
       </div>
      </tr>
    </table>
     <el-pagination
      ...
      axios
        ...
        .then(res => {
          res.data.data.map(item => {y
            ...
            this.$set(item, 'hour', Math.ceil((now - item.time) / 1000 / 60/24y))
